...
class ApiViewTests(TestCase):
    ...
    def test_autofollow_complete_friendrequest(self):
        ...
        request_dict = {"author":self.user_profile.as_dict(),"friend":self.user_profile2.as_dict()}
        request_data = json.dumps(request_dict)
        request = self.factory.post('/api/friendrequest/', body=request_data)
        ...
        request = self.factory.post('/api/friendrequest/', request_data)
        response = friend_request(request)
    ...
    def test_already_following_friendrequest(self):
        ...
        Follow.objects.create(follower=self.user_profile,following=self.user_profile2)
        firstFound = Follow.objects.filter(Q(follower=authorId,following=friendId) | Q(following=friendId,follower=authorId))

        request_dict = json.dumps({"author":self.user_profile.as_dict(),"friend":self.user_profile2.as_dict()})
        request = self.factory.post('/api/friendrequest/',content_type='application/json', data=request_dict)

        print request.body

        response = friend_request(request)
        found = Follow.objects.filter(Q(follower=authorId,following=friendId) | Q(following=friendId,follower=authorId))
